...
retrieval_questions: ["Subagents (서브에이전트)는 어떻게 설정하는가?", "Subagents (서브에이전트) 초기 구성 절차는?", "Subagents (서브에이전트)의 /agents 명령으로 생성 (권장)은 무엇인가?", "Subagents (서브에이전트)의 마크다운 파일로 수동 생성은 무엇인가?", "Subagents (서브에이전트)의 Output Format은 무엇인가?"]
