...
| 2 | 저장소 정합성 6종 (`check_consistency.py`) | 차단 |
...
confidence 필터 정책 문서 일치, 정정 원장 회귀 시험, **병합 보존 불변식 자기시험**을
묶은 것이다. 마지막 항목은 병합 함수가 조각을 버리도록 바꿔 놓고 빌드를 돌려
`build_v3.py`의 assert가 살아 있는지 확인한다 — T-10이 코드로 지켜지는 방식이다.
